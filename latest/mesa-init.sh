#!/bin/bash
# MESA per-user startup: iRODS env, Data Store dotfiles, .env files, OSN/S3 mounts.
# SOURCE this from the entrypoint so exported .env variables reach the app.
# add irods config
mkdir -p $HOME/.irods
...
# These files will be mounted to $HOME/ by the K8s CSI driver.
